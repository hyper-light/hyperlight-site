...
The README records `resolve`, `OutlineExtractor`, and `FileStat` among the respective results. You can go straight to those definitions instead of working through a directory of possible matches. [CLI setup and examples.][getting-started]
...
This work is language-specific. Compiling a grammar into the binary gives us a parser, not complete resolution for that language. The [language matrix][languages] records what is actually extracted and resolved.
...
The encoder adds a download, memory use, and query time. In the README's quality runs, it improves results on CPython and Vorpal's own repository but scores lower on the kernel. Those runs precede background embedding fill, and the default and optional tiers were measured on different days.
...
All results come from the [README benchmarks][readme] on an Apple M5 Max with 18 cores and 128 GB RAM, macOS 26.4.1, and rustc 1.98.0. The main indexing and competitor comparisons used v0.9.0 on September 7, 2026. Broader language and optional-tier results include September 5–6 runs; the agent experiments use v0.8.3. The kernel checkout is `1590cf032971`, CPython is `b86a41cbf63`, and the README pins the other repositories too.
...
These measurements precede the encoder's background embedding fill. In those runs, descriptive kernel queries and paraphrases remained weak; the reranker can't fix a missing candidate. The default column is from September 7, after body-text candidates were added; learned and encoder columns are from September 6. These are the published configurations at those stages. Use `vorpal tune` to test your own questions. [Retrieval results and labels.][search-quality]
